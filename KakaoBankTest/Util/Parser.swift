...
class Parser: NSObject {
    ...
    static func getResultAsset(with results:[SearchResult]?) -> Observable<[SearchResult]> {
        ...
        return Observable.create { (observer) -> Disposable in
            ...
            let success = {
                ...
                } else if assetResults.count % 5  == 0 {
                    observer.onNext(assetResults)
                }
            }
                    ...
                    for (i,_url) in screenshoturls.enumerated() {
                        ...
                        DispatchQueue.global(qos: .userInteractive).sync {
                            guard let url = URL(string: _url),
                                let data = try? Data(contentsOf: url, options: .uncached),
                                let screenshot = UIImage(data: data, scale: 0.1) else {
                                    return
                            }
